...
if (isset($_POST['submit'])) {
    ...
    $imageErr = $_FILES['image']['tmp_name'];
    ...
    if (count($error) == 0) {
        $conn1 = mysqli_connect($host, $username, $sqlpassword, $dbname);
        if (!$conn1->connect_errno) {
//             $query = "INSERT INTO users (name, email, password  , gender ) 
//  VALUES  ('" . $nameErr . "'  , '"  . $hashpassword  . "' , '" . $emailErr  . "' ,'" . $DBgender . "' ) ";
        ...
        $stmt = $conn->prepare("INSERT INTO users (name, email, password  , gender ) 
VALUES (?, ?, ?)");
